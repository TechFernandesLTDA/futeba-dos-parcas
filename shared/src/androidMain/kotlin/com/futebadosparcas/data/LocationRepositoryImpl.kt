...
/**
 * Implementacao Android do LocationRepository.
 *
...
class LocationRepositoryImpl(
    ...
    override suspend fun getLocationsWithPagination(
        ...
        return try {
            PlatformLogger.d(TAG, "Buscando locais com paginacao: limit=$limit, last=$lastLocationName")
    ...
    override suspend fun getLocationsByOwner(ownerId: String): Result<List<Location>> {
        return try {
            ...
            PlatformLogger.d(TAG, "Cache MISS: buscando locais do proprietario: $ownerId")
    ...
    override suspend fun getLocationById(locationId: String): Result<Location> {
        return try {
            ...
            PlatformLogger.d(TAG, "Cache MISS: buscando local: $locationId")
    ...
    override suspend fun getServerLocationVersion(locationId: String): Result<Location> {
        return try {
            PlatformLogger.d(TAG, "Buscando versao do servidor (bypass cache) para local: $locationId")
    ...
    override suspend fun searchLocations(query: String): Result<List<Location>> {
        return try {
            ...
            PlatformLogger.d(TAG, "Cache MISS: buscando locais: $query")
    ...
    override suspend fun seedGinasioApollo(): Result<Location> {
        return try {
            PlatformLogger.d(TAG, "Fazendo seed do Ginasio Apollo")
    ...
    override suspend fun getLocationsPaginated(
        ...
        return try {
            PlatformLogger.d(TAG, "Buscando locais paginados: pageSize=$pageSize, cursor=$cursor, sortBy=$sortBy")
            firebaseDataSource.getLocationsPaginated(pageSize, cursor, sortBy)
        } catch (e: Exception) {
            PlatformLogger.e(TAG, "Erro ao buscar locais paginados", e)
            Result.failure(e)
        }
    }
     ...
     * Util para refresh forçado ou logout.
     */
